-- ---------------------------------------------------------------------------
-- Withdrawal lifecycle: lock on submit, deduct on admin approval.
--
-- user_token_balances.balance is the FREE portion; locked_balance is held
-- separately (matches the trade-place pattern in 0030). On submit we move
-- amount from free -> locked; on approve we drain it from locked permanently.
--
--   * request_withdrawal -> balance -= amount, locked_balance += amount
--   * approve_withdrawal -> locked_balance -= amount   (committed/spent)
--   * mark_withdrawal_paid -> status only (no balance change)
--   * reject_withdrawal pending  -> balance += amount, locked_balance -= amount
--   * reject_withdrawal approved -> balance += amount  (lock already drained)
--   * cancel_withdrawal pending  -> balance += amount, locked_balance -= amount
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- Move the requested amount from free balance into locked. Total holdings
  -- (balance + locked_balance) are unchanged; only the available portion drops.
...
-- ---------------------------------------------------------------------------
-- approve_withdrawal: now performs the actual balance deduction.
-- ---------------------------------------------------------------------------
...
begin
  ...
  if v_withdrawal.token_id is not null and v_withdrawal.amount is not null then
    -- Free balance was already debited at submit; approval drains the lock so
    -- the funds are no longer part of the user's total holdings.
    update public.user_token_balances
...
-- ---------------------------------------------------------------------------
-- reject_withdrawal: release lock if pending; refund balance if approved.
-- ---------------------------------------------------------------------------
...
begin
  ...
  if v_withdrawal.token_id is not null and v_withdrawal.amount is not null then
    if v_was_status = 'pending' then
      -- Move the held amount back from locked into free.
      update public.user_token_balances
      ...
      -- Already approved: locked was drained at approve, so just refund free.
      update public.user_token_balances
      set balance    = balance + v_withdrawal.amount,
          updated_at = now()
      where user_id = v_withdrawal.user_id and token_id = v_withdrawal.token_id;
...
-- ---------------------------------------------------------------------------
-- cancel_withdrawal: only valid while pending; releases the lock.
-- ---------------------------------------------------------------------------
