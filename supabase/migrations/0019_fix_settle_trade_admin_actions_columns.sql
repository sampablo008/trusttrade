-- Fix: settle_trade and bulk_settle_trades wrote to admin_actions columns that
-- do not exist on the table (admin_id/action/after_json/notes). The real
-- columns are admin_user_id/action_type/after_state/note (see 0001_init.sql).
-- Every call raised "column does not exist" and rolled back the entire
-- transaction, so no trade was ever actually settled — the UI was removing
-- rows optimistically against a failed RPC.
