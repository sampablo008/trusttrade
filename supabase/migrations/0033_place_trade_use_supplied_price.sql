-- ---------------------------------------------------------------------------
-- Promote p_lock_price_usd_cents to the PRIMARY price source for place_trade.
--
-- Rationale: cached DB columns (last_price_cents / last_shadow_price_cents)
-- depend on edge functions (tick_candles, shadow_fetch) running on schedule.
-- When those are idle, the previous fallback chain bottoms out at
-- base_price_cents (a low placeholder) and the stake math demands far more
-- token than the user actually has.
--
-- The trade-execution path now requires the caller (lib/trades/service.ts)
-- to resolve a fresh USD price (CoinGecko/Binance) and pass it in. The DB
-- chain remains as a defensive fallback only.
-- ---------------------------------------------------------------------------
