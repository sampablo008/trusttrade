-- ---------------------------------------------------------------------------
-- Fix place_trade INSERT: column was renamed in migration 0015 from
-- trade_period_id → period_id, but every subsequent re-creation of
-- place_trade (0027, 0030, 0031, 0032, 0033) kept the old name in the
-- INSERT clause. plpgsql doesn't validate column references at function
-- creation time, so the bug only surfaces when a trade is actually placed.
-- ---------------------------------------------------------------------------
